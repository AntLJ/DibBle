...
HPALETTE DibPalUniformGrays(int iNum)
{
	...
	for (i = 0; i < iNum; i++)
	{
		plp->palPalEntry[i].peRed = plp->palPalEntry[i].peGreen = plp->palPalEntry[i].peBlue = (BYTE)(i * 255 / (iNum - 1));
		plp->palPalEntry[i].peFlags = 0;
	}
	hPalette = CreatePalette(plp);
	free(plp);
...
HPALETTE DibPalMedianCut(HDIB hdib, int iRes)
{
	...
	qsort(boxes, iEntry, sizeof(BOXES), (_CoreCrtNonSecureSearchSortCompareFunction)Compare);
